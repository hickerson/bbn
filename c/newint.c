/* newint.f -- translated by f2c (version 20100827).
...
/* Right after a run. */
	xout_ref(flags_1.it, 8) = xout_ref(flags_1.it, 8) + xout_ref(
		flags_1.it, 9);
/* Add beryllium to lithium. */
	xout_ref(flags_1.it, 5) = xout_ref(flags_1.it, 5) + xout_ref(
		flags_1.it, 4);
/* Add tritium to helium-3. */
	xout_ref(flags_1.it, 6) = xout_ref(flags_1.it, 6) - 3e-4;
/* my correction for fitted rates+coarse steps */
	...
	do_fio(&c__1, (char *)&outdat_1.etaout[flags_1.it - 1], (ftnlen)
		sizeof(doublereal));
	do_fio(&c__1, (char *)&xout_ref(flags_1.it, 3), (ftnlen)sizeof(
		doublereal));
	do_fio(&c__1, (char *)&xout_ref(flags_1.it, 5), (ftnlen)sizeof(
		doublereal));
	do_fio(&c__1, (char *)&xout_ref(flags_1.it, 6), (ftnlen)sizeof(
		doublereal));
	do_fio(&c__1, (char *)&xout_ref(flags_1.it, 8), (ftnlen)sizeof(
		doublereal));
	e_wsfe();

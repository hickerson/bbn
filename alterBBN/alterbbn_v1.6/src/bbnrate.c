...
/* H + n -> g + H2 */
	if(T9<1.5) 
        f[pn_gd] = 44060.*(1.-2.7503695564153143*T9-3.5220409333117897*T9*T9
	...
	{
		if(T9<1.5) 
            ferrlow[pn_gd] = -(67.95212182165885*sqrt(1.255364135304537+618.8966688079934*T9
                ...
                -111071.90152148822*pow(T9,7./2.)-44154.571125115704*pow(T9,9./2.)))/f[pn_gd];
		else 
            ferrlow[pn_gd]=-0.078;
...
/* He4 + a + n -> g + Be9 */
	f[aan_gBe9]=2.59e-6/((T9*0.344+1.)*(T9*T9))*exp(-1.062/T9);
	
/* He4 + 2a -> g + C12 */
	f[aaa_gC12]=pow(T9,-1.5)*2.79e-8*pow(T9,-1.5)*exp(-4.4027/T9)+pow(T9,-1.5)*1.35e-8*exp(-24.811/T9);
	
/* Li8 + p -> n + a + He4 */
	f[Li8p_naa]=pow(T9,-2./3.)*8.65e9*exp(-8.52/pow(T9,1./3.)-T9/2.53*T9/2.53)+pow(T9,-1.5)*2.31e9*exp(-4.64/T9);
	
/* B8 + n -> p + a + He4 */
	f[B8n_paa]=4.02e8;
	
/* Be9 + p -> d + a + He4 */
	f[Be9p_daa]=pow(T9,-2./3.)*2.11e11*exp(-10.359/pow(T9,1./3.)-T9/0.52*T9/0.52)*(pow(T9,1./3.)*0.04+1.+pow(T9,2./3.)*1.09+T9*0.307+pow(T9,4./3.)*3.21+pow(T9,5./3.)*2.3)+5.79e8/T9*exp(-3.046/T9)+pow(T9,-0.75)*8.5e8*exp(-5.8/T9);
	
/* B11 + p -> 2 a + He4 */
	f[B11p_aaa]=pow(T9,-2./3.)*2.2e12*exp(-12.095/pow(T9,1./3.)-T9/1.644*T9/1.644)*(pow(T9,1./3.)*0.034+1.+pow(T9,2./3.)*0.14+T9*0.034+pow(T9,4./3.)*0.19+pow(T9,5./3.)*0.116)+pow(T9,-1.5)*4.03e6*exp(-1.734/T9)+pow(T9,-1.5)*6.73e9*exp(-6.262/T9)+3.88e9/T9*exp(-14.154/T9);

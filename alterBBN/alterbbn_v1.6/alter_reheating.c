	...
	if(argc<6) 
	...
	else 
  	{
   		sscanf(argv[1],"%lf",&dd0);
     		...
     		sscanf(argv[3],"%lf",&Sigmad0);
     		sscanf(argv[4],"%lf",&nSigmad);
    		sscanf(argv[5],"%lf",&Trh);
  	}
	
	Init_cosmomodel(&paramrelic);	

	Init_dark_density(dd0,ndd,Trh,&paramrelic);
	Init_dark_entropySigmaD(Sigmad0,nSigmad,Trh,&paramrelic);
